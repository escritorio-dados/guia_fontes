...
export interface ICol<T extends IObjectWithId> {
  header: string;
  key?: string;
  minWidth?: string;
  maxWidth?: string;
...
interface ICustomTable<T extends IObjectWithId> {
  cols: Array<ICol<T>>;
  data: T[];
  pagination?: {
    totalPages: number;
                ...
                <TableRowStyled component="div" key={row.id}>
                  {cols.map((col) =>
                    col.customColumn != null ? (
                      ...
                      >
                        {col.key != null ? row[col.key] : 'Informação não encontrada'}
                      </TableCellData>
                    ),
                  )}
                </TableRowStyled>
